...
def scan(f, init, xs, out, checkpoint_group=0):
    ...
    def scan_fn(carry, i_start, i_end):
        for i in range(i_start, i_end):#循环minibatch，串行输入得到新的W，再把新的W用于下一个输入
            if isinstance(xs, dict):
...
class TTTBase(nn.Module):
    ...
    def _init_qkvo_proj(self):
        ...
        # 上式输出维度/2，目的是匹配梯度的维度
        # self.o_proj = nn.Linear(self.width, self.num_heads * self.head_dim // 2, bias=False)
...
class TTTLinear(TTTBase):
    ...
    def ttt(
        ...
        if last_mini_batch_params_dict is not None:
            ...
            init_params_dict = {
                "W1_states": torch.tile(self.W1.unsqueeze(0), dims=(B, 1, 1, 1)),#根据bs复制tensor，使其维度能对上，相当于expand？
...
class TTT(nn.Module):
    ...
    def forward(
        ...
        # get last_mini_batch_params_dict
        last_mini_batch_params_dict = self.last_mini_batch_params_dict
        # TTT Layer
...
class LSTM_Transformer(nn.Module):
    def __init__(self, config: TTTConfig, d_model=256):
        super().__init__()
        self.hidden_size = config.hidden_size

        
        self.seq_modeling_block = nn.LSTM(d_model, d_model, 1, batch_first=True)
    ...
    def forward(
        ...
        # TTT Layer
        hidden_states, _ = self.seq_modeling_block(hidden_states)
...
class LSTM(nn.Module):
    def __init__(self, config: TTTConfig, d_model=256):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.seq_modeling_block = nn.LSTM(d_model, d_model, num_layers = 1, batch_first=True)
...
class Attn(nn.Module):
    def __init__(self, config: TTTConfig, d_model=256):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.seq_modeling_block = nn.MultiheadAttention(embed_dim=d_model, num_heads=8)
    ...
    def forward(
        ...
        # TTT Layer
        hidden_states, _ = self.seq_modeling_block(hidden_states,hidden_states,hidden_states)
        _ = None
        return hidden_states,_
    
class Attn_Transformer(nn.Module):
    def __init__(self, config: TTTConfig, d_model=256):
        super().__init__()
        self.hidden_size = config.hidden_size

        
        # self.seq_modeling_block = nn.GRU(d_model, d_model, num_encoder_layers, batch_first=True)
...
class GRU_Transformer(nn.Module):
    def __init__(self, config: TTTConfig, d_model=256, num_encoder_layers=2):
        super().__init__()
        self.hidden_size = config.hidden_size
    ...
    def forward(
        ...
        # TTT Layer
        hidden_states, _ = self.seq_modeling_block(hidden_states,hidden)
...
if __name__ == "__main__":
    ...
    configuration = TTTConfig(hidden_size=768, intermediate_size=2048, num_hidden_layers=12, num_attention_heads=12, mini_batch_size=1,)
    # configuration.mini_batch_size = 1
    ttt_metanet = TTT(configuration, 0)
    ttt_metanet.eval()
    ...
    for _ in range(50):
        ...
        outputs, last_mini_batch_params_dict = ttt_metanet(inputs, last_mini_batch_params_dict=None)
        optimizer.zero_grad()
    ...
    for i in range(50):
        ...
        for i in range(10):
            ...
            outputs_i, last_mini_batch_params_dict = ttt_metanet(inputs_i, last_mini_batch_params_dict=None)
