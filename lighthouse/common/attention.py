...
class Attention(nn.Module):
    def __init__(self, num_heads=8, d_model=256, bias=False, eps=1e-6):
...
class MultiLayerAttention(nn.Module):
    def __init__(self, num_layers=4, num_heads=8, d_model=256, bias=False, eps=1e-6):
        super(MultiLayerAttention, self).__init__()
        self.layers = nn.ModuleList([
            Attention(num_heads=num_heads, d_model=d_model, bias=bias, eps=eps) 
            for _ in range(num_layers)
        ])

    def forward(self, x):
        for layer in self.layers:
            x = layer(x)  # 将输入依次传入每一层
        return x