...
class BaseOptions(object):
    ...
    def parse(self, yaml_path, domain, base_path=None):
        opt = {}
        ...
        opt.domain = domain
        if opt.query != "unify":
            opt.results_dir = os.path.join(opt.results_dir, str(opt.query)) 
        if opt.domain:
            opt.results_dir = os.path.join(opt.results_dir, opt.domain)
            opt.ckpt_filepath = os.path.join(opt.results_dir, "checkpoint")
